...
use eRecept\Request\CancelPrescriptionRequest;
...
use eRecept\Response\ZrusitPredpisResponse;

class CancelRequestFunctionTest extends FunctionTest
{
	...
	public function testClient()
	{
		...
		$cancelPrescription = new CancelPrescription(
			...
			$this->userPersonalData->getUserUid(),
			'00000000000',
			$createResponse->getDocumentId(),
			new \DateTimeImmutable('now', new \DateTimeZone('Europe/Prague')),
			'TEST',
			$createResponse->getMessageId()
		);
		$response = $this->getClient(false)->send(new CancelPrescriptionRequest($cancelPrescription));

		$this->assertInstanceOf(ZrusitPredpisResponse::class, $response);
